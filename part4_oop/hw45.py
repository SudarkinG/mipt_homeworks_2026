...
class LFUPolicy(Policy[K]):
    ...
    def get_key_to_evict(self) -> K | None:
        ...
        candidates = [key for key in self._key_counter if key != last_added_key]
        if not candidates:
            return None
        min_requests = min(self._key_counter[key] for key in candidates)
        for key in self._key_counter:
            if key in candidates and self._key_counter[key] == min_requests:
                return key
        return None
...
class CachedProperty[V]:
    ...
    def __get__(self, instance: HasCache[Any, V] | None, owner: type) -> "V | CachedProperty[V]":
        if instance is None:
            return self
